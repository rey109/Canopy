...
export default function TopBar() {
  ...
  return (
    <header className="sticky top-0 z-40 flex items-center justify-between px-4 py-3 border-b border-[var(--border)] bg-[var(--bg-secondary)]/95 backdrop-blur-md md:hidden">
      {/* Brand */}
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-[9px] text-[var(--text-muted)] leading-none">{user?.group_name} • {user?.role_name}</p>
        </div>
      </div>

      {/* Right actions */}
      <div className="flex items-center gap-2">
        <Link href="/dashboard/updates" className="relative p-2 rounded-xl hover:bg-[var(--bg-primary)] transition-colors">
        ...
        <button onClick={handleLogout} className="p-2 rounded-xl hover:bg-[var(--bg-primary)] transition-colors">
